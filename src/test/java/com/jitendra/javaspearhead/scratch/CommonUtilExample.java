...
public class CommonUtilExample {
    ...
    private void printMap(Map<String, List<Tuple>> map) {
        ...
        map.keySet().forEach(key ->{
            log.info("\n"+key+"\n");
            map.get(key).forEach(value -> log.info( String.valueOf(value.t)));
        });
    }
    ...
    private void printTable(List<Tuple> tuples) {
        ...
        for (int i = 0; i < tuples.size(); i++) {
            String token = (String) tuples.get(i).t;
            String pos = (String) tuples.get(i).j;
            String prominant =(String) tuples.get(i).k;
